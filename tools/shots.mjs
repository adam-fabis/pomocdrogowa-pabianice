...
// Użycie: node tools/shots.mjs [port] [ścieżka...]   → .superpowers/shots/<strona>-<szerokość>.png
import { chromium } from 'playwright';
import fs from 'node:fs';
const port = process.argv[2] || '8080';
const paths = process.argv.slice(3).length ? process.argv.slice(3) : ['/', '/oferta/', '/galeria/', '/kontakt/'];
const widths = [390, 1280];
fs.mkdirSync('.superpowers/shots', { recursive: true });
...
for (const w of widths) {
  ...
  for (const p of paths) {
    ...
    await page.screenshot({ path: `.superpowers/shots/${name}.png`, fullPage: true });
